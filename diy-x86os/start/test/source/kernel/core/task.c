...
void task_first_init (void) {
    ...
    // 第一个任务代码量小一些，好和栈放在1个页面内
    // 这样就不要立即考虑还要给栈分配空间的问题
    task_init(&task_manager.first_task, "first task", 0, first_start, first_start + alloc_size);
    task_manager.first_task.heap_start = (uint32_t)e_first_task;  // 这里不对
    task_manager.first_task.heap_end = task_manager.first_task.heap_start;   //末端地址等于起始地址
...
/*
在给出的代码中，确实是这样的逻辑：父进程在执行 `fork()` 函数时会返回子进程的进程ID，
而在 `sys_fork()` 函数中，子进程的 `tss` 结构中的 `eax` 寄存器被设置为0，
这样在子进程开始执行后(任务切换），它会通过读取 `eax` 寄存器来获取返回值。
*/
int sys_fork (void) {
    ...
    //系统调用的时候，会自动向特权级为0的栈中压入一些参数，然后会手动压入一些参数
    //这些参数全部构成结构体syscall_frame_t
    //由于栈指针从高地址开始，所以减去大小就是起始地址
    //其中的eip就保存了执行系统调用前函数执行的位置
    //esp需要恢复到父进程手动压栈之前（也就是那五个参数）
...
static uint32_t load_elf_file (task_t * task, const char * name, uint32_t page_dir) {
    ...
    if ((elf_hdr.e_type != 2) || (elf_hdr.e_machine != 3) || (elf_hdr.e_entry == 0)) {
